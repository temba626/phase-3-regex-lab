...
describe "Working with Regular expressions" do
  describe "#starts_with_a_vowel?" do
    it "returns true for words starting with a vowel" do
      ...
      match.each { |word| expect(starts_with_a_vowel?(word)).to be(true) }
    end
  ...
  describe "#words_starting_with_un_and_ending_with_ing" do
    it "returns an array with the words starting with 'un' and ending with 'ing'" do
      words_string =
        "unassuming ambiguous understanding pomp circumstance uninteresting uncompromising grouchy unruly corollary uninterestingly"

      expect(
        words_starting_with_un_and_ending_with_ing(words_string).count
      ).to eq(4)
      expect(
        words_starting_with_un_and_ending_with_ing(words_string)
      ).to include("understanding")
      expect(
        words_starting_with_un_and_ending_with_ing(words_string)
      ).not_to include("pomp")
    end
...
"
  ...
  describe "#valid_phone_number?" do
    it "returns true for valid phone numbers, regardless of formatting" do
      ...
      expect(valid_numbers.all? { |number| valid_phone_number?(number) }).to be(
        true
      )
    end
